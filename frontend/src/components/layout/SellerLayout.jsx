...
import {
  ...
  LogOut, Menu, Sun, Moon, Sparkles, PlusCircle
} from 'lucide-react';
...
export default function SellerLayout() {
  ...
  const isActive = (link) => link.exact ? location.pathname === link.to : location.pathname === link.to;
    ...
    <div className="flex h-screen bg-gray-50 dark:bg-gray-950 overflow-hidden">
      <aside className={`${sidebarOpen ? 'w-60' : 'w-16'} flex-shrink-0 bg-white dark:bg-gray-900 border-r border-gray-100 dark:border-gray-800 flex flex-col transition-all duration-300`}>
        <div className="h-16 flex items-center px-4 border-b border-gray-100 dark:border-gray-800 gap-3">
          <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-secondary-500 to-accent-500 flex-shrink-0 flex items-center justify-center">
            <Sparkles className="w-4 h-4 text-white" />
          </div>
          {sidebarOpen && <span className="font-display font-bold text-secondary-600 dark:text-secondary-400">Seller Hub</span>}
        </div>
        ...
        <header className="h-16 bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800 flex items-center px-4 gap-4">
          ...
          <div className="flex-1">
            <h1 className="font-semibold text-gray-900 dark:text-white">
              {links.find(l => isActive(l))?.label || 'Seller Dashboard'}
            </h1>
          </div>
          ...
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-secondary-500 to-accent-500 flex items-center justify-center text-white text-sm font-bold">
              {user?.name[0]}
            </div>
